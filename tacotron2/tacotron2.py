...
from tensorflow_addons.rnn import GRUCell, MultiRNNCell, OutputProjectionWrapper, ResidualWrapper,LSTMStateTuple
...
class Tacotron2():
    ...
    def initialize(self, inputs, input_lengths, num_speakers, speaker_id=None,mel_targets=None, linear_targets=None, is_training= False,loss_coeff=None,stop_token_targets=None):
        ...
        with tf.compat.v1.variable_scope('Eembedding') as scope:
            hp = self._hparams
            batch_size = tf.shape(inputs)[0]

            # Embeddings(256)
            char_embed_table = tf.compat.v1.get_variable('inputs_embedding', [len(symbols), hp.embedding_size], dtype=tf.float32,initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.5))
            ...
            if self.num_speakers > 1:
                speaker_embed_table = tf.compat.v1.get_variable('speaker_embedding',[self.num_speakers, hp.speaker_embedding_size], dtype=tf.float32,initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.5))
                # [N, T_in, speaker_embedding_size]
                speaker_embed = tf.nn.embedding_lookup(speaker_embed_table, speaker_id)                       
                
                deep_dense = lambda x, dim,name: tf.compat.v1.layers.dense(x, dim, activation=tf.nn.softsign,name=name)   # softsign: x / (abs(x) + 1)
        ...
        with tf.compat.v1.variable_scope('Encoder') as scope:
            ...
            for i in range(hp.enc_conv_num_layers):
                x = tf.compat.v1.layers.conv1d(x,filters=hp.enc_conv_channels,kernel_size=hp.enc_conv_kernel_size,padding='same',activation=tf.nn.relu,name='Encoder_{}'.format(i))
                x = tf.compat.v1.layers.batch_normalization(x, training=is_training)
                x = tf.compat.v1.layers.dropout(x, rate=hp.dropout_prob, training=is_training, name='dropout_{}'.format(i))
            ...
            outputs, states = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw,encoder_conv_output,sequence_length=input_lengths,
        ...
        with tf.compat.v1.variable_scope('Decoder') as scope:
            ...
            decoder_lstm = tf.compat.v1.nn.rnn_cell.MultiRNNCell(decoder_lstm, state_is_tuple=True)
            ...
            for i in range(hp.postnet_num_layers):
                ...
                x = tf.compat.v1.layers.conv1d(x,filters=hp.postnet_channels,kernel_size=hp.postnet_kernel_size,padding='same',activation=activation,name='Postnet_{}'.format(i))
                x = tf.compat.v1.layers.batch_normalization(x, training=is_training)
                x = tf.compat.v1.layers.dropout(x, rate=hp.dropout_prob, training=is_training, name='Postnet_dropout_{}'.format(i))
 
            residual = tf.compat.v1.layers.dense(x,hp.num_mels,name='residual_projection')
            ...
            linear_outputs = tf.compat.v1.layers.dense(post_outputs, hp.num_freq,name='linear_spectogram_projection')    # [N, T_out, F(1025)]
    ...
    def add_loss(self):
        ...
        with tf.compat.v1.variable_scope('loss') as scope:
            hp = self._hparams
            before = tf.math.squared_difference(self.mel_targets, self.decoder_mel_outputs)
            after = tf.math.squared_difference(self.mel_targets, self.mel_outputs)
